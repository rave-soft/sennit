...
// subscribers (e.g. a TUI watching workspace events while `braid
...
// non-nil error from `braid run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `braid run` waits) exits cleanly rather
...
// queue scenario: `braid run --continue` attaches to a session
