...
// Overridable via BRAID_SERVER_IDLE_TIMEOUT (seconds; 0 restores the
...
// via BRAID_SERVER_DETACH_GRACE (seconds; 0 restores immediate teardown).
...
// Backend provides transport-agnostic business logic for the Braid
...
func New(ctx context.Context, cfg *config.ConfigStore, shutdownFn ShutdownFunc) *Backend {
	return &Backend{
		...
		detachGrace: durationFromEnv("BRAID_SERVER_DETACH_GRACE", DefaultDetachGrace),
	}
}

// idleShutdownDelayFromEnv returns the idle-shutdown delay, honoring a
// BRAID_SERVER_IDLE_TIMEOUT override (in seconds; 0 disables lingering).
func idleShutdownDelayFromEnv() time.Duration {
	return durationFromEnv("BRAID_SERVER_IDLE_TIMEOUT", DefaultIdleShutdownDelay)
}
