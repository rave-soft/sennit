...
func (t *runTurn) finishAssistantOnStreamError(err error, isCancelErr bool) error {
	...
	if isCancelErr {
		...
		switch class := classifyStreamError(providerErr); {
		case class == classRateLimited:
			// The step spent its whole retry budget on 429s, so the turn
			// is done. Say so in words the user can act on rather than
			// passing the provider's own empty body through.
			t.currentAssistant.AddFinish(
				message.FinishReasonError,
				time.Now().Unix(),
				"Rate limited",
				cmp.Or(providerErr.Message, "The provider refused this request with a rate limit (429) and kept refusing it until the retry budget ran out. Wait a while before trying again."),
			)
		case class == classModelNotEnabled && t.model.ModelCfg.Provider == string(catwalk.InferenceProviderCopilot):
