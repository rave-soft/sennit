...
// A file bigger than MaxReadSize must be returned truncated at the cap
...
func TestReadTextFile_TruncatesAtSizeCapInsteadOfFailing(t *testing.T) {
	...
	for range 300 { // ~300KB > MaxReadSize
		b.WriteString(line)
		b.WriteByte('\n')
	}
	require.NoError(t, os.WriteFile(path, []byte(b.String()), 0o644))

	content, hasMore, err := readTextFile(path, 0, 100000, MaxReadSize)
	...
	require.LessOrEqual(t, len(content), MaxReadSize)
