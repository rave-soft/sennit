...
func TestBuildAnthropicProvider(t *testing.T) {
	t.Run("plain API key uses X-Api-Key, not Authorization", func(t *testing.T) {
		...
		provider, err := c.buildAnthropicProvider(server.URL, "akey", map[string]string{}, "some-provider", "")
		require.NoError(t, err)
	...
	t.Run("MiniMax China also forces the Bearer wrapping", func(t *testing.T) {
		...
		c := newProxyTestCoordinator(t, false)
		server, captured := newCaptureServer(t)
		provider, err := c.buildAnthropicProvider(server.URL, "akey", map[string]string{}, string(catwalk.InferenceProviderMiniMaxChina), "")
		require.NoError(t, err)
		probe(t, provider, "abab6.5")
		require.Equal(t, "Bearer akey", captured.Header.Get("Authorization"))
	})
	...
	t.Run("extra headers merge into the request", func(t *testing.T) {
		...
		c := newProxyTestCoordinator(t, false)
		server, captured := newCaptureServer(t)
		provider, err := c.buildAnthropicProvider(server.URL, "akey", map[string]string{"X-Custom": "v"}, "some-provider", "")
		require.NoError(t, err)
		probe(t, provider, "claude-3")
	...
	t.Run("invalid proxy URL surfaces an error and no provider", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildAnthropicProvider("", "akey", map[string]string{}, "some-provider", "://bad")
		require.Error(t, err)
...
func TestBuildOpenaiProvider(t *testing.T) {
	...
	t.Run("plain provider sends Bearer auth to the configured base URL", func(t *testing.T) {
		...
		c := newProxyTestCoordinator(t, false)
		server, captured := newCaptureServer(t)
		provider, err := c.buildOpenaiProvider(server.URL, "akey", map[string]string{"X-Custom": "v"}, "openai", "")
		require.NoError(t, err)
	...
	t.Run("Codex provider ID wraps the transport without changing the request", func(t *testing.T) {
		...
		c := newProxyTestCoordinator(t, false)
		server, captured := newCaptureServer(t)
		provider, err := c.buildOpenaiProvider(server.URL, "akey", map[string]string{}, codex.ProviderID, "")
		require.NoError(t, err)
	...
	t.Run("invalid proxy URL surfaces an error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildOpenaiProvider("", "akey", map[string]string{}, "openai", "://bad")
		require.Error(t, err)
		require.Nil(t, provider)
	})
}
...
func TestBuildOpenaiCompatProvider(t *testing.T) {
	...
	t.Run("plain provider sends Bearer auth", func(t *testing.T) {
		...
		c := newProxyTestCoordinator(t, false)
		server, captured := newCaptureServer(t)
		provider, err := c.buildOpenaiCompatProvider(server.URL, "akey", map[string]string{}, nil, "some-vendor", false, "")
		require.NoError(t, err)
	...
	t.Run("extra body fields are injected into the request", func(t *testing.T) {
		...
		provider, err := c.buildOpenaiCompatProvider(server.URL, "akey", map[string]string{}, extraBody, "some-vendor", false, "")
		require.NoError(t, err)
		probe(t, provider, "some-model")
	...
	t.Run("Copilot provider ID adds the X-Initiator header via its own transport", func(t *testing.T) {
		...
		c := newProxyTestCoordinator(t, false)
		server, captured := newCaptureServer(t)
		provider, err := c.buildOpenaiCompatProvider(server.URL, "akey", map[string]string{}, nil, string(catwalk.InferenceProviderCopilot), false, "")
		require.NoError(t, err)
	...
	t.Run("invalid proxy URL surfaces an error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildOpenaiCompatProvider("http://example.test", "akey", map[string]string{}, nil, "some-vendor", false, "://bad")
		require.Error(t, err)
		require.Nil(t, provider)
	})
}
...
func TestBuildAzureProvider(t *testing.T) {
	...
	t.Run("API key goes through Api-Key, not Authorization", func(t *testing.T) {
		...
		c := newProxyTestCoordinator(t, false)
		server, captured := newCaptureServer(t)
		provider, err := c.buildAzureProvider(server.URL, "akey", map[string]string{"X-Custom": "v"}, nil, "")
		require.NoError(t, err)
		probe(t, provider, "gpt-4o")
	...
	t.Run("invalid proxy URL surfaces an error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildAzureProvider("http://example.test", "akey", map[string]string{}, nil, "://bad")
		require.Error(t, err)
		require.Nil(t, provider)
	})
}
...
func TestBuildGoogleProvider(t *testing.T) {
	...
	t.Run("API key goes through X-Goog-Api-Key", func(t *testing.T) {
		...
		c := newProxyTestCoordinator(t, false)
		server, captured := newCaptureServer(t)
		provider, err := c.buildGoogleProvider(server.URL, "akey", map[string]string{"X-Custom": "v"}, "")
		require.NoError(t, err)
	...
	t.Run("invalid proxy URL surfaces an error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildGoogleProvider("http://example.test", "akey", map[string]string{}, "://bad")
		require.Error(t, err)
		require.Nil(t, provider)
	})
}
...
func TestBuildOpenrouterAndVercelProvider(t *testing.T) {
	...
	t.Run("openrouter: construction wires all options without error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildOpenrouterProvider("ignored-base-url", "akey", map[string]string{"X-Custom": "v"}, "")
		require.NoError(t, err)
	...
	t.Run("openrouter: invalid proxy URL surfaces an error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildOpenrouterProvider("", "akey", map[string]string{}, "://bad")
		require.Error(t, err)
		require.Nil(t, provider)
	})
	...
	t.Run("vercel: construction wires all options without error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildVercelProvider("ignored-base-url", "akey", map[string]string{"X-Custom": "v"}, "")
		require.NoError(t, err)
		require.NotNil(t, provider)
	})
	...
	t.Run("vercel: invalid proxy URL surfaces an error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildVercelProvider("", "akey", map[string]string{}, "://bad")
		require.Error(t, err)
		require.Nil(t, provider)
	})
}
...
func TestBuildBedrockProvider(t *testing.T) {
	t.Run("explicit API key builds without error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildBedrockProvider("akey", map[string]string{"X-Custom": "v"}, "some-provider", "")
		require.NoError(t, err)
		require.NotNil(t, provider)
	})
	...
	t.Run("AWS_BEARER_TOKEN_BEDROCK env fallback builds without error", func(t *testing.T) {
		...
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildBedrockProvider("", map[string]string{}, "some-provider", "")
		require.NoError(t, err)
		require.NotNil(t, provider)
	})
	...
	t.Run("bedrock-europe and the default both build without error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildBedrockProvider("akey", map[string]string{}, string(catwalk.InferenceProviderBedrockEurope), "")
		require.NoError(t, err)
		require.NotNil(t, provider)
	})
	...
	t.Run("invalid proxy URL surfaces an error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildBedrockProvider("akey", map[string]string{}, "some-provider", "://bad")
		require.Error(t, err)
		require.Nil(t, provider)
	})
}
...
func TestBuildGoogleVertexProvider(t *testing.T) {
	...
	t.Run("construction wires project/location and headers without error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildGoogleVertexProvider(map[string]string{"X-Custom": "v"}, map[string]string{"project": "proj", "location": "us-central1"}, "")
		require.NoError(t, err)
		require.NotNil(t, provider)
	})
	...
	t.Run("invalid proxy URL surfaces an error", func(t *testing.T) {
		t.Parallel()
		c := newProxyTestCoordinator(t, false)
		provider, err := c.buildGoogleVertexProvider(map[string]string{}, nil, "://bad")
		require.Error(t, err)
		require.Nil(t, provider)
	})
}
...
func TestBuildProvider(t *testing.T) {
	...
	t.Run("openai type routes to buildOpenaiProvider", func(t *testing.T) {
		...
		provider, err := c.buildProvider(providerCfg, config.SelectedModel{Model: "gpt-4"}, false)
		require.NoError(t, err)
		probe(t, provider, "gpt-4")
		require.Equal(t, "Bearer akey", captured.Header.Get("Authorization"))
	})
	...
	t.Run("anthropic type adds interleaved-thinking beta header for a thinking model", func(t *testing.T) {
		...
		provider, err := c.buildProvider(providerCfg, model, false)
		require.NoError(t, err)
		probe(t, provider, "claude-3")
	...
	t.Run("anthropic type without thinking does not add the beta header", func(t *testing.T) {
		...
		server, captured := newCaptureServer(t)
		providerCfg := config.ProviderConfig{Type: "anthropic", APIKey: "akey", BaseURL: server.URL}
		provider, err := c.buildProvider(providerCfg, config.SelectedModel{Model: "claude-3"}, false)
		require.NoError(t, err)
		probe(t, provider, "claude-3")
	...
	t.Run("OpenCode provider ID routes its Messages-API model list to the anthropic builder", func(t *testing.T) {
		...
		provider, err := c.buildProvider(providerCfg, config.SelectedModel{Model: "qwen3.7-max"}, false)
		require.NoError(t, err)
	...
	t.Run("OpenCode provider ID with a non-Messages model stays on openaicompat", func(t *testing.T) {
		...
		provider, err := c.buildProvider(providerCfg, config.SelectedModel{Model: "some-other-model"}, false)
		require.NoError(t, err)
	...
	t.Run("azure type routes to buildAzureProvider", func(t *testing.T) {
		...
		provider, err := c.buildProvider(providerCfg, config.SelectedModel{Model: "gpt-4o"}, false)
		require.NoError(t, err)
		probe(t, provider, "gpt-4o")
		require.Equal(t, "akey", captured.Header.Get("Api-Key"))
	})
	...
	t.Run("google type routes to buildGoogleProvider", func(t *testing.T) {
		...
		provider, err := c.buildProvider(providerCfg, config.SelectedModel{Model: "gemini-2.5-pro"}, false)
		require.NoError(t, err)
		probe(t, provider, "gemini-2.5-pro")
		require.Equal(t, "akey", captured.Header.Get("X-Goog-Api-Key"))
	})
	...
	t.Run("openrouter type routes to buildOpenrouterProvider", func(t *testing.T) {
		...
		provider, err := c.buildProvider(providerCfg, config.SelectedModel{Model: "some-model"}, false)
		require.NoError(t, err)
		require.NotNil(t, provider)
	})
	...
	t.Run("a known custom provider type falls back to openaicompat", func(t *testing.T) {
		...
		provider, err := c.buildProvider(providerCfg, config.SelectedModel{Model: "llama3"}, false)
		require.NoError(t, err)
	...
	t.Run("an unknown provider type is rejected", func(t *testing.T) {
		...
		provider, err := c.buildProvider(providerCfg, config.SelectedModel{Model: "m"}, false)
		require.Error(t, err)
		require.Nil(t, provider)
