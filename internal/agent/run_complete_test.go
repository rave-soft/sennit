...
// event into a closure nobody reads, and subscribers (`sennit run`)
...
// publish a RunComplete for its RunID, hanging a `sennit run` caller that
...
// than internal bookkeeping, which is the contract a `sennit run` caller
...
// bookkeeping slice, not the published event a `sennit run` caller awaits.
