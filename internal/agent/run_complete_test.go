...
// event into a closure nobody reads, and subscribers (`braid run`)
...
// publish a RunComplete for its RunID, hanging a `braid run` caller that
...
// than internal bookkeeping, which is the contract a `braid run` caller
...
// bookkeeping slice, not the published event a `braid run` caller awaits.
