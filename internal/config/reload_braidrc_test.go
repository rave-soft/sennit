...
// isolateReloadEnv points HOME/XDG/SENNIT_* at a throwaway directory so a
...
// already behaves.
func TestReloadFromDisk_PicksUpEditedSennitrc(t *testing.T) {
	...
	require.Equal(t, "osc", store.Config().Options.Notifications,
		"reload must re-execute the sennitrc and pick up the new value")
}

// TestReloadFromDisk_FailingSennitrcKeepsOldConfig verifies that a sennitrc
...
func TestReloadFromDisk_FailingSennitrcKeepsOldConfig(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	rcPath := filepath.Join(workDir, "sennitrc")
	require.NoError(t, os.WriteFile(rcPath, []byte("option notifications bell\n"), 0o644))
	...
	// Overwrite with a sennitrc that errors at load time (unknown option key).
	require.NoError(t, os.WriteFile(rcPath, []byte("option totally-bogus-key value\n"), 0o644))

	err = store.ReloadFromDisk(context.Background())
	require.Error(t, err, "a failing sennitrc must surface a reload error")
	require.Equal(t, "bell", store.Config().Options.Notifications,
		"in-memory config must be preserved when a reload fails")
}

// TestReloadFromDisk_HangingSennitrcIsInterruptible is the core regression test
// for the reload-path blocker: config reloads run while holding the store's
// write lock, so a sennitrc that blocks (busy loop, hung command substitution)
...
func TestReloadFromDisk_HangingSennitrcIsInterruptible(t *testing.T) {
	...
	// Overwrite with a sennitrc that never terminates.
...
// TestLoad_TracksNotYetCreatedGlobalSennitrc verifies that config.Load tracks
// the global sennitrc path even when the file does not exist yet, so a sennitrc
// created after startup is detected as a staleness change. Previously only
// successfully-loaded paths were tracked, so a mid-session global sennitrc went
// unnoticed until something else triggered a reload.
//
// Project-level sennitrc files are discovered by walking the tree for existing
// files, so a not-yet-created project sennitrc is still only picked up on the
// next reload; the global path is the common case this covers.
func TestLoad_TracksNotYetCreatedGlobalSennitrc(t *testing.T) {
	workDir, dataDir := isolateReloadEnv(t)
	globalRC := filepath.Join(t.TempDir(), "sennitrc")
	t.Setenv("SENNIT_GLOBAL_CONFIG", filepath.Dir(globalRC))
	...
	// Create the global sennitrc after startup.
	require.NoError(t, os.WriteFile(globalRC, []byte("option debug true\n"), 0o644))

	staleness := store.ConfigStaleness()
	require.True(t, staleness.Dirty, "creating a global sennitrc must be detected")
