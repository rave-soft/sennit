...
You review Go code.`)
	...
	got := discoverMarkdownAgents(root)
...
func TestDiscoverSkipsPrimaryMode(t *testing.T) {
	...
	require.NotContains(t, discoverMarkdownAgents(root), "build")
}
...
func TestDiscoverIgnoresMissingDirs(t *testing.T) {
	require.NotPanics(t, func() { discoverMarkdownAgents(t.TempDir()) })
	require.Empty(t, discoverMarkdownAgents(""), "an unknown working dir must not scan anything")
}
