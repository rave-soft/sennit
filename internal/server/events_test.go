...
// are what non-interactive clients (e.g. `braid run`) rely on to
...
// through the SSE envelope. Without these correlators, `braid run`
...
// "agent failed" (returns non-zero from `braid run`) from "agent
