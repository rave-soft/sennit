...
- Do not poll for the result. A task's outcome — completed, failed, or
  ...
  id; sibling completions may arrive in any order. `agent_result` is for
...
- A workspace can turn background tasks off entirely
  (`options.background_agents`). When it is, ordinary task creation is refused;
  the agent_* management tools remain available if the workspace has isolated
  delegations.
...
to `agent_cancel` is refused rather than obeyed: it once was not, and the
...
- `agent_list` — the tasks you started, at any depth, and their statuses.
- `agent_result` — a specific task's outcome once it has one.
- `agent_send` — queue a follow-up message into a task's own session. Refused
  once the task has been cancelled.
- `agent_cancel` — stop a task's in-flight run, and the runs of every task it
...
- `agent_output` — a tail of a task's own transcript (user/assistant text
...
only go through the agent_* tools above, which run in the same process as the
task itself.
