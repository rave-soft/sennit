...
// Handle is a running workspace hosting one strand's isolated app.App:
// its own .braid data directory, database, and agent coordinator, rooted
// at the strand's git worktree.
type Handle interface {
	...
	ID() string
	// App is the strand's isolated application instance.
	App() *app.App
}

// Spawner bootstraps and tears down the isolated workspace backing a
// strand. Implementations differ in how the resulting workspace's
// lifecycle interacts with the rest of the process — see [LocalSpawner]
// for the in-process CLI case and internal/backend/strand_spawner.go for
// the client/server case.
type Spawner interface {
	// Spawn bootstraps an app.App rooted at path (a strand's git
...
// LocalSpawner spawns strand workspaces by bootstrapping a plain
