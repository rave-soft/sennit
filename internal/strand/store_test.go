...
func TestStore_CreateAndGet(t *testing.T) {
	...
	require.Equal(t, "strand/alpha", created.Branch)
	require.Equal(t, "/tmp/strands/alpha", created.WorktreePath)
...
func TestStore_List(t *testing.T) {
	...
	strands, err := store.List(t.Context())
	require.NoError(t, err)
	require.Len(t, strands, 2)
	// ListStrands orders by created_at, so insertion order is preserved.
	require.Equal(t, first.ID, strands[0].ID)
	require.Equal(t, second.ID, strands[1].ID)
}
