...
type Service interface {
	...
	ListUnfinishedAssistantMessages(ctx context.Context, projectPath string) ([]message.Message, error)
...
type pendingState struct {
	// latest is the most recent [message.Message] passed to [Service.Update]
	...
	// without comparing message.Message values, whose Parts may contain slices.
	generation uint64
...
func (s *service) Create(ctx context.Context, sessionID string, params message.CreateMessageParams) (message.Message, error) {
	if params.Role != message.Assistant {
		params.Parts = append(params.Parts, message.Finish{
			Reason: "stop",
		})
	}
	partsJSON, err := message.MarshalParts(params.Parts)
	if err != nil {
...
func shouldFlushNow(prev, next *message.Message) bool {
	if next.IsFinished() {
		return true
	}

	var prevCalls []message.ToolCall
...
func messagesFrom[T any](rows []T, convert func(T) (message.Message, error)) ([]message.Message, error) {
...
func (s *service) Get(ctx context.Context, id string) (message.Message, error) {
...
func (s *service) List(ctx context.Context, sessionID string) ([]message.Message, error) {
...
func (s *service) ListUnfinishedAssistantMessages(ctx context.Context, projectPath string) ([]message.Message, error) {
	...
	return messagesFrom(rows, func(row db.ListUnfinishedAssistantMessagesRow) (message.Message, error) {
...
func (s *service) ListUserMessages(ctx context.Context, sessionID string) ([]message.Message, error) {
...
func (s *service) ListBySessionIDs(ctx context.Context, sessionIDs []string) (map[string][]message.Message, error) {
	if len(sessionIDs) == 0 {
		return nil, nil
	}
...
func (s *service) fromDBItem(item db.Message) (message.Message, error) {
	parts, err := message.UnmarshalParts([]byte(item.Parts), item.ID)
	if err != nil {
		return message.Message{}, err
	}
