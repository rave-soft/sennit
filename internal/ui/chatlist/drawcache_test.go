...
func TestChatDrawCache_HitOnIdenticalRender(t *testing.T) {
	...
	_ = renderToBuffer(t, c)
	require.NotNil(t, c.drawCache, "first draw should populate cache")
	firstCache := c.drawCache
	firstBuf := c.drawCache.buf.RenderBuffer

	_ = renderToBuffer(t, c)
	require.Same(t, firstCache, c.drawCache,
		"identical rendered string must reuse the same cache pointer")
	require.Same(t, firstBuf, c.drawCache.buf.RenderBuffer,
...
func TestChatDrawCache_MissOnDifferentRender(t *testing.T) {
	...
	got := renderToBuffer(t, c)
	require.NotSame(t, firstCache, c.drawCache,
	...
	want := freshStyledRender(c.RenderList(), 80, h)
...
func TestChatDrawCache_ReusedAcrossDifferentArea(t *testing.T) {
	...
	// hold the list width fixed and only shrink
	...
	c.Draw(smallScr, uv.Rect(0, 0, 40, 10))
	...
	want := freshStyledRender(c.RenderList(), 40, 10)
...
func TestChatDrawCache_BoundedSize(t *testing.T) {
	...
	for i := range 5 {
		...
		_ = renderToBuffer(t, c)
		require.NotNil(t, c.drawCache)
		require.Equal(t, c.RenderList(), c.drawCache.rendered,
	...
	require.IsType(t, (*chatDrawCache)(nil), c.drawCache)
}
...
func TestChatDrawCache_InvalidatedByWidthMethodSwap(t *testing.T) {
	...
	c.Draw(scrA, uv.Rect(0, 0, w, h))
	require.NotNil(t, c.drawCache)
	require.Equal(t, ansi.GraphemeWidth, c.drawCache.method)
	firstCache := c.drawCache
	...
	c.Draw(scrB, uv.Rect(0, 0, w, h))
	require.NotSame(t, firstCache, c.drawCache,
		"width method change must invalidate the cache")
	require.Equal(t, ansi.WcWidth, c.drawCache.method)
}
...
func TestChatDrawCache_FallbackOnNonAnsiMethod(t *testing.T) {
	...
	want := freshStyledRender(c.RenderList(), w, h)
...
func TestRenderedBounds_MatchesPrintStringTallyForZWJ(t *testing.T) {
	...
	for _, m := range []ansi.Method{ansi.GraphemeWidth, ansi.WcWidth} {
		...
		require.Equal(t, gotW, w,
			"chatlist.RenderedBounds width must match printString cell tally for method %v", m)
		...
		require.Equal(t, gotH, h,
			"chatlist.RenderedBounds height must match printString row tally for method %v", m)
	}
}
