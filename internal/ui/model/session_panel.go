...
// splitTodosActiveCompleted splits todos into the active set — in-progress
// todos first, then pending, each preserving relative order — and the
// completed set. This is the opposite of chat.FormatTodosList's
// completed-first ordering: that function renders a single tool call's
// transcript, where reading top-down chronologically makes sense; the
// panel is a persistent status view, where what's happening now belongs
// above what's already done.
func splitTodosActiveCompleted(todos []session.Todo) (active, completed []session.Todo) {
	...
	active = append(inProgress, pending...)
...
type sessionPanelPlan struct {
	...
	todosTotal     int
	todosActive    []session.Todo // shown when todosExpanded
...
func (m *UI) sessionPanelPlan(budget int) sessionPanelPlan {
	...
	if plan.todosVisible {
		...
		if plan.todosExpanded {
			plan.todosActive, plan.todosDone = splitTodosActiveCompleted(todos)
		}
	}
	...
	todosRows := func() int {
		...
		return 1 + len(plan.todosActive) + len(plan.todosDone)
	}
	over := func() int {
		return plan.threadsRows + todosRows() + len(plan.queue) - budget
	}
	...
	plan.totalRows = plan.threadsRows + todosRows() + len(plan.queue)
	return plan
}

// sessionPanelHeight reports how many rows the merged session panel needs,
// capped at sessionPanelBudgetFraction of the terminal height. generateLayout
// further clamps this against the space actually available in mainRect,
// mirroring the pre-existing dockHeight clamp.
func (m *UI) sessionPanelHeight() int {
	budget := max(0, int(float64(m.height)*sessionPanelBudgetFraction))
