...
func TestDrawSessionPanel_RendersEveryThreadBlock(t *testing.T) {
	...
	require.Len(t, u.panel.threadRects, 6)
	require.Len(t, u.panel.threads, 6)
}
...
func TestSessionPanelPlan_QueueAlwaysVisibleRegardlessOfTodosExpand(t *testing.T) {
	...
	u.sess.current.Todos = []session.Todo{{Status: session.TodoStatusPending, Content: "x"}}
	u.panel.expanded = true
...
func TestMouseClick_ThreadBlockEntersThread(t *testing.T) {
	...
	require.Len(t, u.panel.threadRects, 1)

	rect := u.panel.threadRects[0]
...
// m.panel.todosHeaderRect, which is only populated as a side effect of
...
func TestDrawSessionPanel_TodosScrollRevealsHiddenRows(t *testing.T) {
	...
	require.Equal(t, maxOffset, u.panel.todosScrollOffset, "wheel-down must reach the section's bottom")
	...
	require.Equal(t, maxOffset, u.panel.todosScrollOffset, "offset must clamp at the bottom")
}
...
func TestRenderSessionTodoLine_CompletedStyleSurvivesBudgetConstrainedPlan(t *testing.T) {
	...
	maxOffset := plan.todosContentRows - plan.todosViewportRows
	u.panel.todosScrollOffset = maxOffset
	scr := uv.NewScreenBuffer(u.lay.width, u.lay.height)
	u.drawSessionPanel(scr, u.lay.layout.panel)
	out := scr.Render()
...
// allSectionsUI builds a UI with every visible section populated: an
// active thread, an expanded todos list with both incomplete and completed
// items, and a queued prompt — the fixture the section-separator-header
// tests below share.
func allSectionsUI(t *testing.T) *UI {
	t.Helper()
...
// TestDrawSessionPanel_AllSectionsGetSeparatorHeaders covers the panel
...
func TestDrawSessionPanel_AllSectionsGetSeparatorHeaders(t *testing.T) {
	t.Parallel()
	...
	require.Less(t, threadsHeaderIdx, todosHeaderIdx)
...
// populated, threadsHeaderRows/queueHeaderRows must
...
// its own section-separator header above it.
func TestMouseClick_ThreadBlockEntersThreadBelowItsHeader(t *testing.T) {
	t.Parallel()
...
func TestMouseClick_TodosHeaderTogglesWithHeaderStyling(t *testing.T) {
	...
	scr := uv.NewScreenBuffer(u.lay.width, u.lay.height)
	u.drawSessionPanel(scr, u.lay.layout.panel)
	require.NotZero(t, u.panel.todosHeaderRect)

	rect := u.panel.todosHeaderRect
	_, cmd := u.Update(tea.MouseClickMsg{X: rect.Min.X, Y: rect.Min.Y, Button: tea.MouseLeft})
