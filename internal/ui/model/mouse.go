...
func (m *UI) updateMouse(msg tea.Msg, cmds []tea.Cmd) ([]tea.Cmd, bool) {
	...
	case chatlist.DelayedClickMsg:
		cmds = m.applyChatDelayedClick(msg, cmds)
	case tea.MouseClickMsg:
		return m.applyMouseClick(msg, cmds)
	case tea.MouseMotionMsg:
		return m.applyMouseMotion(msg, cmds)
	case tea.MouseReleaseMsg:
		return m.applyMouseRelease(msg, cmds)
	case common.CoalescedWheelMsg:
		return m.applyMouseWheel(msg, cmds)
	}
	return cmds, false
}

// applyChatDelayedClick handles chatlist.DelayedClickMsg: resolves a
// delayed single-click action (e.g., expansion, or navigating into a
// clicked child-session delegation). messageID and toolCallID come from the
// clicked item directly, not m.chat.SelectedNestedToolContainer — a mouse
// click doesn't move the keyboard-driven selection that reads (see
// HandleMouseDown). enterChildSession returns nil for a delegation that has
// not started yet (see delegationStarted): the click is simply not one, so
// nothing is queued for it.
func (m *UI) applyChatDelayedClick(msg chatlist.DelayedClickMsg, cmds []tea.Cmd) []tea.Cmd {
...
// applyMouseClick handles tea.MouseClickMsg: routes the click to dialogs, an
// active inline editor, the header's threads badge, the breadcrumb bar's
// Back button, the session panel's hit-testable rows, an attachment chip's
// remove button, or finally the chat list itself, in that priority order.
func (m *UI) applyMouseClick(msg tea.MouseClickMsg, cmds []tea.Cmd) ([]tea.Cmd, bool) {
...
// applyMouseMotion handles tea.MouseMotionMsg: dialog hover, the breadcrumb
// bar and attachment chip hover states, session panel hover, inline-editor
// hover tracking, and — while in chat — scrollbar drag, text-selection
// edge-scroll, and drag/hover forwarding to the chat list.
func (m *UI) applyMouseMotion(msg tea.MouseMotionMsg, cmds []tea.Cmd) ([]tea.Cmd, bool) {
...
// applyMouseRelease handles tea.MouseReleaseMsg: ends a scrollbar drag, or
// on a plain mouse-up with an active chat highlight, arms the
// double-click-detection tick that copyChatHighlightMsg resolves.
func (m *UI) applyMouseRelease(msg tea.MouseReleaseMsg, cmds []tea.Cmd) ([]tea.Cmd, bool) {
	// Pass mouse events to dialogs first if any are open.
	if m.dialog.HasDialogs() {
		m.dialog.Update(msg)
		return cmds, true
...
// applyMouseWheel handles common.CoalescedWheelMsg: routes wheel events to
// an active inline editor when hovered, then to dialogs, then — while in
// chat — to sidebar scrolling, the session panel's todos scroll, horizontal
// shell-output scroll, or the chat list itself, keeping the selection
// synced with the resulting viewport.
func (m *UI) applyMouseWheel(msg common.CoalescedWheelMsg, cmds []tea.Cmd) ([]tea.Cmd, bool) {
