...
type AgentEvent struct {
	...
	// `sennit run`) attribute an error event to a specific request
	...
	// TypeAWSSSOAuthResult so the refresh dialog can show live progress.
	// The command runs in the coordinator; these ferry its progress to the
	// UI. AWSSOCommand is the refresh command being run; AWSSOURL is the
	// verification URL once it appears in the command output. The result's
	// failure text travels through Error, like TypeAgentError.
