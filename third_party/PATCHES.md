...
| `pkg/transport/connection.go` | `Connection.Close` no longer calls `jsonrpc2.Conn.Close()` directly. `sourcegraph/jsonrpc2` v0.2.1's `Conn.close()` clears every pending call's `done` channel without deleting it from `c.pending`, and its read loop (`readMessages`) delivers a response by deleting the pending entry and then writing to, and closing, that same channel - outside any lock. Calling `Close()` from a goroutine other than the read loop, concurrently with a response delivery, could make both touch the same channel and panic on a double close. `Close` now closes the raw process stream first (a new `stream` field, set from `NewConnection`'s `stream` argument): that fails the read loop's next `Read`, and the read loop calls `Conn.close()` itself on its own goroutine, never racing its own delivery - the same self-close already used by the graceful shutdown path (`Client.Exit`, which never calls `conn.Close()` at all). `Close` waits on `jsonrpc2.Conn.DisconnectNotify()` (a new `disconnect` field) for that self-close to finish, falling back to a direct `conn.Close()` only if it doesn't happen within `disconnectWait` - which reopens the original race, but only in that unexpected case, instead of always. |
