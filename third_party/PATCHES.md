...
| `agent.go` | `executeTools` now stops running further tool calls in a batch as soon as one result carries `StopTurn`, synthesizing an error result for each call it skips instead of running it, rather than only consulting `StopTurn` afterwards via `hasStopTurn` to decide whether to make another model call - Sennit's `hooked_tool.go` (a PreToolUse hook halt), `tools/tools.go` (a denied permission) and `tools/question.go` (a user question) all set `StopTurn` to mean "stop now", so a blocked `bash` call previously still let a sibling `write` call in the same step execute. |
...
| `pkg/transport/connection.go`, `pkg/lsp/client.go` | `Connection.Close` no longer calls `jsonrpc2.Conn.Close()` directly. `sourcegraph/jsonrpc2` v0.2.2's `Conn.close()` clears every pending call's `done` channel without deleting it from `c.pending`, and its read loop (`readMessages`) delivers a response by deleting the pending entry and then writing to, and closing, that same channel - outside any lock. Calling `Close()` from a goroutine other than the read loop, concurrently with a response delivery, could make both touch the same channel and panic on a double close. `NewConnection` gives jsonrpc2 a background context so its cancellation watcher cannot independently invoke `Conn.close`; `Connection.Close` owns forced teardown by closing the raw process stream, killing and reaping the child, then waiting for the read loop to self-close. Graceful `Exit` now waits first for disconnect and then, within the same deadline, for normal process exit and reap. The stream/process closer separates peer-EOF pipe cleanup and graceful `Wait` from forced `ForceClose`, so a cooperative server may close its transport, finish bounded cleanup, and exit normally without being killed. If either graceful wait expires, forced close interrupts the pipes, kills, and reaps the child. The code never calls `jsonrpc2.Conn.Close()` from an external goroutine, including fallback paths. |

Sennit's own test for the `router.go` fix lives in `pkg/transport/router_test.go`, a file of its own added by Sennit (not part of the powernap baseline). The `connection.go` teardown-ordering fix is exercised end-to-end from `internal/lsp` (a hung LSP server torn down via the timeout/`Kill()` path), in `internal/lsp/lifecycle_test.go`, rather than as a unit test in this tree.
