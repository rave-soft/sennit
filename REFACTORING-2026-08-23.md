...
Принцип, по которому шла работа: сначала выписать различия между копиями,
потом решать, что выносить. Молча унифицированное различие — это не
дедупликация, а изменение поведения.

- `DelegationParent`: пять мест различались источником session ID,
  координатором (внешний родительский против собственного), источником
  `ParentSessionID` и depth. Все различия сохранены параметрами хелпера.
- `discover`: единственное существенное различие enricher'ов — стрипается ли
  `/v1` из базового URL, поэтому URL остался аргументом вызывающего.
  `error` из `EnrichModels` убран с доказательством: ни одна из пяти
  реализаций не могла вернуть non-nil, все три вызывающих его игнорировали.
  `ollama.go` под общий скелет не подошёл (POST с телом, конкурентные
  запросы) и оставлен как есть.
- `fsext`: два списка игнорируемых каталогов слиты, но НЕ полным
  объединением. `generated` и `logs` выброшены намеренно — их скрытие
  делало невозможным поиск по сгенерированному коду и чтение логов;
  `OrbStack`, `.local`, `.share` выброшены как осмысленные только
  относительно `$HOME`. Причины записаны в доккомментарии, обратное
  поведение закреплено тестами. `SkipHidden` больше не аллоцирует мапу на
  каждый вызов из glob.
- OAuth-диалоги: общий диалог НЕ сделан. Различия принципиальны (6/4/3
  состояния, разное владение асинхронщиной, разная отмена и клавиши);
  вынесены только байт-в-байт совпадающие куски рендера.
- `lineViewport`: сброс compositor'а кнопок при уходе строки из окна стал
  структурным свойством (`Blit` вызывает колбэк только для видимых строк),
  а не починкой в одном месте.
- Два форматтера времени сведены к `presentation.FormatElapsed`. Таймер хода
  в чате теперь `3m20s` вместо `3m 20s` — осознанно, это формат, которым уже
  пользуются остальные восемь мест.
- Ширины в `chat`: капирование теперь ровно в одном месте (`tools_item.go`),
  тело инструмента и hook-индикатор совпадают (было расхождение в 2 колонки),
  добавлен guard от отрицательных ширин. Единственный перегенерированный
  голден — `TestMouseGolden/bash_tool_expand_click`, где 7 строк потеряли по
  2 колонки правого паддинга: ровно то, что и означает исправление.


- [x] `thread/manager.go:326,595,732` + `tasks.go:220,416` — конструирование
  `DelegationParent` повторено 5 раз с расхождениями (в одном из них родился
  баг Depth); один хелпер `registerParent`. **S**
- [x] `discover/{lmstudio,llamacpp,litellm,omlx}.go` — четыре enricher'а на
  одном копипаст-скелете (~40 строк × 4); generic `fetchJSON[T]` + чистые
  apply-функции; заодно убрать мёртвый `error` из `EnrichModels`. **M**
- [x] `tools/bash.go:258-300 vs 330-372` — два почти идентичных блока
  завершения (~45 строк), расходятся молча. **S**
- [x] `fsext/lookup.go` — четыре функции = две пары одинаковых замыканий;
  `fsext` — три независимых ignore-списка с дрейфом, мапа `SkipHidden`
  аллоцируется на каждый вызов из glob. **S**
- [x] UI-дубли: markdown-рендер вопроса ×6 в dialog; «line-buffer → clamp →
...
- [x] Двойной `cappedMessageWidth` во всех tool-рендерерах
...
- [x] Фаза 5, пункт 11: вторая половина («extract `pendingState` into a
  channel-driven `messageWriter`») **сделана (2026-08-23), но не буквально.**
  Busy-wait `time.Sleep(time.Millisecond)` убран: ожидающий забирает под
  локом ссылку на канал текущей попытки флаша (`pendingState.flushDone`) и
  блокируется на нём либо на `ctx.Done()`; канал закрывается ровно один раз
  под тем же локом, что исключает и потерю сигнала, и двойное закрытие.
  Отдельная горутина-writer НЕ заводилась: вся координация и так под
  `service.mu`, а лишняя горутина потребовала бы своего протокола
  остановки без выигрыша. «Flush before read» оказался не контрактом,
  протекающим к вызывающим, а внутренней гонкой — она и закрыта; задержка
  дебаунса при чтении осознанная и сохранена.
- [x] Фаза 1: `diffview/style.go:28` — `DefaultLightStyle` числится
  удалённым, но существует. **Разобрано (2026-08-23): претензия неверна.**
  Функция жива и нужна: это светлый парник к `DefaultDarkStyle`
  (продакшн-умолчание в `diffview.go:91`), и на ней построены все
  LightMode-голдены пакета — 8 вызовов в `diffview_test.go`. Удалять
  нечего, ошибочна была отметка «удалено» в предыдущем плане.
- [x] Фаза 0: `workspace/resolve_session.go:44` — клиентский скан с TODO
  «expose GetLast» **закрыт (2026-08-23).** Запрос `GetLastSession` уже
  существовал, но был неограничен; добавлен фильтр
  `parent_session_id IS NULL` (та же область, что у `ListSessions`),
  метод выведен на `Workspace`, скан удалён, sqlc перегенерирован.
  Дополнительная проверка `ParseAgentToolSessionID` из скана не
  переносилась: суб-сессии всегда несут `parent_session_id`, так что она
  недостижима — эквивалентность, а не тождество, и тест с нереалистичной
  фикстурой (agent-tool ID при пустом `ParentSessionID`) переписан.
