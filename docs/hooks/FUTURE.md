...
Paths are resolved relative to `BRAID_CWD`. Non-existent paths are dropped with
a debug log (don't fail the hook over a missing file).

### How the agent sees it

Braid appends a short note to the turn's context along the lines of:
...
- Should `context_files` paths be constrained to `BRAID_PROJECT_DIR`? Probably
...
Today Braid supports exactly one hook event, `PreToolUse`. That's enough to gate
