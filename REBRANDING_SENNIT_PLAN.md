...
- [ ] Удалить `LetterB`, `LetterR`, `LetterA` и `LetterD` вместе с их тестами
  после переключения логотипа. В `SENNIT` из старого набора используется
  только `LetterI`, остальные четыре становятся мертвым кодом; если их не
  удалить сознательно, они останутся в дереве навсегда.
- [ ] Нарисовать недостающие ASCII-литеры для большого логотипа. Это не
  пересчет ширины, а работа дизайнера, и она блокирует весь пункт про
  wordmark. `internal/ui/logo/letterforms.go` содержит ровно пять функций —
  `LetterA`, `LetterB`, `LetterD`, `LetterI`, `LetterR`, — то есть буквы
  слова BRAID и ничего больше; `Render` в `logo.go` собирает логотип их
  прямым перечислением. Для `SENNIT` нужны новые `LetterS`, `LetterE`,
  `LetterN` и `LetterT` в том же стиле; `LetterI` переиспользуется.
- [ ] Обновить terminal header, большой wordmark и small logo под шесть букв
  `SENNIT`. Слово на букву длиннее `BRAID`, поэтому ширина и раскладка
  требуют пересчета, а `Render` — перечисления шести литер вместо пяти.
  Малый логотип и шапка берут имя из констант `brand.Name` и
  `brand.Wordmark`, их правка сводится к смене значения.
