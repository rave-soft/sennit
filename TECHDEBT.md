...
- **Неатомарный маркер импорта легаси-базы.** `db.ImportLegacyProjectDB`
  фиксирует импорт в общую базу транзакцией, но переименование `braid.db` →
  `braid.db.imported` — отдельная, не связанная с этой транзакцией файловая
  ...
  запуск увидит тот же `braid.db` и повторит импорт; безвредно благодаря
...
- **Глобальный каталог opencode — предположение.**
  ...
  оно разойдётся с реальным расположением, `braid import opencode --global`
...
Не долг, а контракт `braid import` — описан здесь, потому что на этот раздел
ссылается код (`internal/config/import.go`, скилл `braid-config`).
