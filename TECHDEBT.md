...
- **Хвосты ревью-раунда 3 (2026-08-23).** `REFACTORING-2026-08-23-r3.md`
  ...
  **Границы слоёв.** Закрыто: `hooks ↛ config` (L2), `threadspawn.AppOf`
  ...
  - L9 UI мимо workspace — **операции вынесены 2026-08-30, остались
    ...
    **Осталось одно исключение, и оно настоящее:** `ui/dialog/oauth_codex.go`
    ведёт флоу входа сам — `StartFlow`, `RefreshToken`, `TokensFromDisk`,
    `ProxyFromDisk`, `ValidateProxy`, `FetchModels`. Вынос в лоб плох:
    либо шесть вендорных методов на 65-методный `Workspace`, либо
    переизобретение `OAuthProvider`, который уже и есть шов. Плюс это
    единственный путь, который пользователь не обойдёт при поломке, а
    локально проверяется только фейками (живые тесты под `CODEX_LIVE=1`).
