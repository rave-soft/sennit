...
- **Хвосты ревью-раунда 3 (2026-08-23).** `REFACTORING-2026-08-23-r3.md`
  ...
  **Границы слоёв.** Закрыто: `hooks ↛ config` (L2), `threadspawn.AppOf`
  ...
  - L9 UI мимо workspace — **операции вынесены 2026-08-30, остались
    типы.** Пункт стоит переформулировать, потому что счёт импортов
    оказался неверной метрикой: он смешивал «UI делает бэкендную работу»
    с «UI называет DTO», а это разные вещи, и второе граница разрешает.

    Вынесено за фасад: загрузка кастомных команд (палитра сама обходила
    конфиг-каталоги), проба Docker MCP (палитра **запускала
    `docker mcp version`** подпроцессом с таймаутом), список типов
    провайдеров (диалог тянул движок discovery глубиной 194 пакета ради
    пяти строк), каталог провайдеров (`config.Providers` пересчитывался
    в семи местах, тогда как стор его кэширует и это тот же список, что
    у разрешения модели), диагностика конфига, состояния скиллов и
    **обнаружение встроенных скиллов** (панель читала их с диска сама).
    Плюс DTO: `hooks.HookMetadata` и состояния LSP уехали в `proto` —
    последнее заодно перестало отдавать в UI **живой `*lsp.Client`**,
    который контракт раздавал через алиас типа.

    Проверено 2026-08-30: боевых вызовов работы из `internal/ui` не
    осталось. Из функций конфига живы только `GlobalConfigData()` и
    `ProviderFieldKey()` — чистые строковые хелперы. `question`,
    `permission`, `history`, `stats`, `skills`, `commands`, `message`,
    `session` — только имена типов. `internal/diff` снят как
    не-нарушение: пакет листовой (ноль внутренних зависимостей), а
    считать дифф для показа — работа представления.

    Ушли в ноль целиком: `hooks`, `lsp`, `git`, `discover`. На
    `internal/ui/model` заведено правило depguard против пакетов входа.

    **Осталось одно исключение — и по нему принято решение не делать,
    2026-08-30.** `ui/dialog/oauth_codex.go` (235 строк) и
    `oauth_copilot.go` (171) ведут флоу входа сами: `ProxyFromDisk`,
    `ValidateProxy`, `TokensFromDisk`, `RefreshToken`, `StartFlow`,
    `flow.Wait`, `flow.Close`, `FetchModels`, `AccountID`, `ProviderName`.

    Разобрано трижды, каждый раз с одним исходом, поэтому фиксирую с
    обоснованием, чтобы не открывать заново.

    **Как это выглядело бы правильно.** Не шесть вендорных методов на
    65-методный `Workspace`, а один, отдающий объект флоу:
    `BeginSignIn(ctx, providerID, proxy) (SignInFlow, error)` плюс
    импорт с диска и постсохранение. Контракт без tea-типов; диалог
    оборачивает его в команды. То есть тот же `OAuthProvider`, но слоем
    ниже и без вендорных импортов в UI.

    **Почему не сделано.** Выгода измерена: три файла в
    `internal/ui/dialog` перестают импортировать `oauth/{codex,copilot}`.
    Цена — переписать оба адаптера (≈400 строк) на новый девятиметодный
    контракт, в единственном пути, который пользователь не обойдёт при
    поломке, и который локально проверяется только фейками: живые тесты
    под `CODEX_LIVE=1`.

    Решающий довод — то, что в этом коде уже поймано и записано.
    `OAuthCodex.stopped` существует, потому что esc во время
    «Initializing» когда-то оставлял порт занятым: teardown успевал
    раньше, чем bind, находил `flow == nil` и не закрывал ничего, а
    следующий вход падал на занятом порту. Флоу привязан к одному
    фиксированному порту редиректа, запасного нет. Ровно такие гонки и
    теряются при переносе через новый контракт, и ни один локальный тест
    их не ловит.

    Отдельно стоит записать, что довод «UI не должен вести флоу» здесь
    слабее, чем в остальных случаях этого пункта. Палитра, запускавшая
    `docker mcp version`, и контракт, отдававший живой `*lsp.Client`, —
    это UI, делающий чужую работу по недосмотру. Диалог входа ведёт
    интерактив, и это его работа; чужое в нём — знание вендорного HTTP,
    а не сам флоу.

    Брать стоит целиком и осознанно, а не попутно.

    Что осталось от «расширить depguard»: правило нельзя ставить на
    `internal/ui/**` для `config`/`message`/`session`, пока там живут
    DTO. Вопрос не в границе, а в том, где живут DTO, — и это отдельная
    задача, не эта.
  ...
  **Божественные объекты.** Закрыто: walker вынесен в
  ...
  (18.2k строк non-test в 54 файлах).

  По `internal/ui/model` **сделан первый настоящий шаг 2026-08-30**, и он
  не про строки. `chat.go` (1687 строк, самый большой файл) теперь
  **не называет ничего** из остального пакета — проверено AST-тестом
  `TestChatDoesNotDependOnTheScreen`. Мешали ровно две вещи, и обе
  оказались размещением, а не зависимостью: в `chat.go` жил таймер
  скроллбара **сайдбара** (единственное упоминание `*UI` во всём файле),
  и метод `Chat` возвращал навигационный тип. Таймер уехал в
  `sidebar.go`; метод теперь отдаёт элементы списка, которые обходит, а
  отображение в `childSessionRef` делает навигация.

  **Вынос сделан в тот же день:** компонент живёт в
  `internal/ui/chatlist`, пакет `internal/ui/model` похудел с 18 250 до
  **16 602** строк non-test.

  Оценка «108 ссылок» была завышена — она считала слово `Chat` в прозе.
  Настоящих мест оказалось два (`widgets.go`, `child_session_nav.go`).
  Зато вылезло то, чего оценка не видела: **тесты читали внутренности
  компонента напрямую** — `chat.list`, `chat.drawCache`, пять полей
  скроллбара. Пока всё лежало в одном пакете, это компилировалось.

  Разобрано по существу, а не экспортом полей: то, о чём тест спрашивает
  законно, стало методами (`Offset`, `Selected`, `VisibleItemIndices`,
  `ScrollbarGeometry`, `Resizing`, `PendingClickID` и ещё несколько), а
  тест кэша отрисовки — единственный, кто действительно смотрит на
  внутренности, — уехал в `chatlist`, где они свои. Ему понадобилась
  своя фикстура: он строил **целый `UI`**, чтобы добраться до поля
  `chat`, хотя проверяет только кэш этого компонента.

  Влить в существующий `internal/ui/chat` было нельзя: там уже 15k строк
  и два конфликта имён (`Animate`, `Restyle`) — получился бы god-object
  на новом месте.

  **Следом вынесены ещё два, 2026-08-30: 18 250 → 15 000 строк non-test.**
  Сначала `internal/ui/listcache` — обобщённый TTL-кэш и последовательность
  fetch/apply/backoff, которыми пользуются четыре файла, то есть они не
  принадлежат ни одной фиче. Затем `internal/ui/threads` — дашборд тредов
  (`threads.go`, `admin.go`, `cache.go`), связный кластер, которому после
  выноса кэша не нужно из `ui/model` **ничего**.

  Порядок был не произволен: утащить общий кэш вместе с threads значило
  бы поставить общий механизм внутрь одной фичи.

  Что вылезло по дороге, как и с чатом, — **тесты читали внутренности**:
  `dashboard.width/height/active`, `threadList.cache`. Разобрано методами
  (`Size`, `Active`, `Threads`, `InvalidateCache`), а не экспортом полей.
  Единственное исключение — `ListCache.Cache`: этот тип целиком и есть
  обёртка над кэшем, прятать в нём нечего.

  **Шов исчерпан, и это замер, а не впечатление.** После выноса чата,
  тредов, дока и кэшей в `internal/ui/model` осталось 14 703 строки
  non-test, из которых **13 496 — методы `*UI` и `*Root`**, то есть сама
  модель экрана, разложенная по 38 файлам. Не компоненты, лежащие не в
  том пакете, а один большой экран.

  Остальные 1207 строк — девять файлов, и самые чистые из них
  (`status.go` 130 строк и `filter.go` 108, обе с нулевой связностью;
  `header.go` 180 с одной) слишком малы, чтобы пакет окупил churn.
  `agents_cache.go` (140, нулевая связность) — тот же случай.

  Значит дальнейшее сокращение — это не перенос файлов, а перестройка
  модели: продолжение того, что делала фаза 3 (перенос методов на
  структуры состояния). Другая задача и другая цена; браться за неё под
  видом «вынести ещё пакет» было бы самообманом.

  И один тест пришлось чинить по существу, а не по имени:
  `TestThreadEventDispatchesOneListThreadsCall` выставлял `active`
  напрямую, а `SetActive` при этом ещё и диспатчит обновление. Замена на
  метод оставляла кэш помеченным in-flight, и событие потом не
  диспатчило ничего — тест мерил ноль вместо единицы. Теперь команда
  активации сливается, а счётчик сбрасывается перед замером. `sennit import` **вынесен
