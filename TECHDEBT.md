...
- **Хвосты ревью-раунда 3 (2026-08-23).** `REFACTORING-2026-08-23-r3.md`
  ...
  **не называет ничего** из остального пакета — проверено AST-тестом
  ...
  Цена оставшегося выноса измерена: экспортировать четыре
  неэкспортированных имени, которыми пакет пользуется из `chat.go`
  (`messageItems`, `renderedBounds`, `scrollbarHideDuration`,
  `showScrollbar`), перенести файл с тестами и обновить 108 ссылок. Это
  механика, и она должна быть отдельной правкой — но она **стала
  возможна**, а до этого не была. Тест сторожит именно это: пакет
  компилируется в обе стороны, и один хелпер, за которым потянулись из
  `chat.go`, молча пришивает компонент обратно к экрану. `sennit import` **вынесен
