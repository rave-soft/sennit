...
- **Schema URL указывает на чужой домен.** В `docs/configuration/sennitrc.md` и в
  ...
  правильный (`https://github.com/rave-soft/sennit/...`). Чинить некуда, пока
  не решены домен и место публикации схемы — это часть незакрытых
  организационных решений ребрендинга (домен, URL документации, судьба
  старого репозитория). Следующий шаг: выбрать адрес публикации схемы, затем
  поправить оба примера.
