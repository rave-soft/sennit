...
(Гонка на неохраняемом поле `app.AgentCoordinator` в этом списке была, но
её закрыли позже во второй волне: поле стало приватным под
`agentCoordinatorMu`, чтение — через `App.Coordinator()`, есть
регрессионный тест `coordinator_race_test.go`.)
...
Все пункты §1-§3 закрыты, §4 выполнен в части документирования
(`third_party/PATCHES.md`). CI зелёный на всех трёх ОС
(`301f4c5c`): race, lint, ubuntu, macos, windows.

Первый push после завершения работ CI **не** прошёл: четыре теста
падали только вне Linux, поэтому локальный прогон их не показывал —
гонка доставки completion (тест фиксировал одного конкретного
победителя гонки, которую сам должен был допускать в обе стороны),
`/var` → `/private/var` в путях worktree, инжекция unix-errno в
общий тест `fsext` и зависание на 600 с в `credentials`, где
`t.Setenv("HOME")` не работает на Windows. Все четыре — дефекты
тестов; продакшн-код не менялся.

Открытые вопросы (нужно решение владельца, не техническое):

1. Асимметрия scope у `RemoveRuntimeConfigField` (см. §0) — латентный
   баг, сегодня недостижимый.
2. Шаг ввода прокси для входа в Copilot — UX-решение, затрагивает всех
   пользователей Copilot.
3. Судьба форка `third_party/`: отправлять патчи upstream или перейти
   на `git subtree`. Базовый SHA `powernap` восстановить не удалось —
   это записано в `PATCHES.md` честно, а не заполнено догадкой.
4. Два коммита (`f3190f39`, `dd289ef9`) несут больше, чем говорят их
   сообщения: неудачный коммит оставлял файлы в индексе. Переписать
   историю не дал классификатор прав; процесс исправлен после второго
   случая.
