...
### 1.1 [S] Непроверенный `Close` при скачивании

`internal/agent/tools/download.go:187` — `outFile.Close()` без проверки.
Это единственное место, где всплывёт ошибка сброса буфера на диск;
инструмент отчитается об успехе с неполным файлом. Остальные находки
`errcheck` (`Body.Close`, `Fprint` в stderr) безобидны.
...
### 2.4 [M] `readOnlyWorkspace` написан руками

`internal/workspace/read_only_workspace.go` — 799 строк, 120 методов, из
них 54 тупо форвардят в `w.ws`. Встроить `Workspace` и переопределить
только 46 мутаторов: минус ~400 строк и минус класс ошибок «забыл
прокинуть новый метод контракта».
...
1. 1.1, 1.2 — реальные баги, по полчаса каждый.
2. 2.4 — минус 400 строк без изменения поведения.
3. 3.1, 3.2 — общий `deps` и удаление форвардеров.
