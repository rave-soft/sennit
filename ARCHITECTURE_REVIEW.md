...
| 1.2 lifecycle тредов | частично | Определить attach завершённых тредов и сделать упорядоченный teardown `App` |
...
| 5.5 client/server dogfood | не начато | Ждёт полного закрытия 1.2 |
...
```
Сначала закончить 1.1 и 1.2
  → 5.1: включить replay в CI (блокирует дальнейшие разрезы Run)
  ...
  → 5.2 уже можно делать поверх готового 2.0;
     5.3 строго после 5.2; 5.5 после полного закрытия 1.2
  → Фаза 6 по отдельным продуктовым решениям
```

Ближайший рекомендуемый срез: закрыть хвосты 1.1/1.2, затем выполнить 2.1 и 2.2.
Это завершает критическую фазу и убирает самые частые блокировки UI до более
рискованных кэшей 2.5 и структурного разреза `Update`.
