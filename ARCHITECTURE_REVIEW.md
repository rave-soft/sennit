...
| 3.6–3.7 конкурентность | не начато | Остаются две границы ownership |
...
| 2.3 | Выполнено: `sendMessage` переносит `AgentReadyErr`/`CreateSession`/`AgentRun` в cmd, а FIFO/generation state machine применяет результаты только в `Update`; provider/model/Copilot, `SetConfigField`, Grant/Deny и `SetCompactMode` выполняются в cmd с typed results, stale guards и rollback в `Update` | средний |
...
2.1 (убрать постоянный HTTP probe)
  ...
  → 3.6–3.7: 3.6 отдельным ownership-рефакторингом, 3.7 точечным PR
...
Ближайший рекомендуемый срез: выполнить 2.1 и 2.2. Критическая фаза и
страховочная сетка 5.1 закрыты; следующий приоритет — убрать самые частые
блокировки UI до более рискованных кэшей 2.5 и структурного разреза `Update`.
